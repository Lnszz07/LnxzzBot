...
fakenomor = '089656093383@s.whatsapp.net'
...
const starts = async (client = new WAConnection()) => {
  ...
  client.browserDescription = ["Lnxzz々 Is Here", "Chrome", "3.0"];
  ...
  client.on("open", () => {
    success("2", "Successfully Connected To Wa, Subscribe Lnxzz々");
console.log(color(figlet.textSync(`${spc1}            𝗭𝗮𝗻𝘀 𝗕𝗼𝘁`, {
font: 'Standard',
...
console.log(color(`${spc4}< ================================================== >`, 'cyan'))
    ...
    console.log(color('⎇','red'), color('𝙄 𝙬𝙧𝙤𝙩𝙚 𝙩𝙝𝙞𝙨 𝙨𝙘𝙧𝙞𝙥𝙩 𝙢𝙮𝙨𝙚𝙡𝙛!\n𝙉𝙤𝙩𝙚: 𝘿𝙤𝙣𝙩 𝙛𝙤𝙧𝙜𝙚𝙩 𝙩𝙤 𝙨𝙪𝙗𝙨𝙘𝙧𝙞𝙗𝙚 𝗭𝗮𝗻𝘀𝗟𝗼𝗿𝗱', 'yellow'))
    console.log(color('⎇','red'), color('𝗦𝗼𝘂𝗿𝗰𝗲 𝗖𝗼𝗱𝗲 𝗩𝗲𝗿𝘀𝗶𝗼𝗻:', 'aqua'), color(version))
    console.log(color('⎇','red'), color('𝗕𝘂𝗴? 𝗘𝗿𝗿𝗼𝗿? 𝗦𝘂𝗴𝗴𝗲𝘀𝘁𝗶𝗼𝗻? 𝗩𝗶𝘀𝗶𝘁 𝗛𝗲𝗿𝗲:', 'aqua'), color('https://wa.me/6289656093383'))
  ...
  client.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "add" && mem.includes(client.user.jid)) {
        client.sendMessage(anu.jid, "Hello everyone, am Lnxzz Bot, ready to help you here ㋛︎", "conversation");
      }
...
if (anu.action == 'add' && !mem.includes(client.user.jid)) {
      ...
      if (anu.action == "remove" && !mem.includes(client.user.jid)) {
        ...
        time_wel = moment.tz("Asia/Jakarta").format("HH:mm");
        memeg = mdata.participants.length;
        out = `Selamat Tinggal Beban Grub ${anu_user} 👋`;
        buff = await getBuffer(
